...
function addProduct(product: Product) {
    ...
    confirmationMessage.value.push(`Le produit "${product.name}" a été ajouté avec succès.`);
}

function deleteProduct(id: number) {
    products.value = products.value.filter((product) => product.id !== id);
    confirmationMessage.value.push(`Le produit avec l'id ${id} a été supprimé avec succès.`);
}

function updateProduct(updatedProduct: Product) {
    const product : Product | undefined = products.value.find((product) => product.id === updatedProduct.id);
    ...
    if (product) {
        ...
        confirmationMessage.value.push(`Le produit "${updatedProduct.name}" a été mis à jour avec succès.`);
    }
    else {
        //Cela pourrait arriver, mais il y a très peu de chance.
        errorMessages.value.push(`Le produit avec l'id ${updatedProduct.id} n'a pas été trouvé.`);
